...
/**
 * This filter allows for setting the Content-Type header of a request to set
 * the character encoding scheme.  Because some browsers do not send the
 * character encoding scheme in the Content-Type header, problems can arise when
 * the server uses a default scheme to parse incoming request parameters.  If
 * the default scheme does not happen to match what the clients' default is, the
 * parameters will be parsed incorrectly.  Note that if the client did send an
 * encoding scheme, this filter
 ...
 * <br><br>
 * <li><b>encodingScheme</b> - This is the actual value to set in the header.
 * This can be any valid character encoding scheme value like ASCII, UTF-8,
 * etc.  Required: Yes.  Default: None.</li>
 * </ul>
 * <br>
 ...
 * CharacterEncodingFilter&lt;/filter-class&gt;<br>
 * &nbsp;&nbsp;&lt;init-param&gt;<br>
...
public class CharacterEncodingFilter implements Filter {
  ...
  static {
    try {
      ...
      System.err.println("CharacterEncodingFilter" +
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    encodingScheme = filterConfig.getInitParameter("encodingScheme");
    log.info("encodingScheme = " + encodingScheme);
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
    ...
    if (FilterHelpers.filterPath(request, pathList, pathSpec)) {
      log.info("CharacterEncodingFilter firing...");
      if (request.getCharacterEncoding() == null) {
        if (log.isDebugEnabled()) {
          log.debug("Character encoding was specified in request, not setting");
        }
      } else {
        if (encodingScheme == null) {
          if (log.isDebugEnabled()) {
            log.debug("Encoding is null, so not setting on request");
          }
        } else {
          if (log.isDebugEnabled()) {
            log.debug("Setting encoding scheme: " + encodingScheme);
          }
