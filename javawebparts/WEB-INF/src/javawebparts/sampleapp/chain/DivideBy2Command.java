 ...
 */
public class DivideBy2Command implements Command {


  /**
   * Log instance.
   */
  private static Log log = LogFactory.getLog(DivideBy2Command.class);
  ...
  public Result init(ChainContext chainContext) {
    ...
    log.info("\n\n**START**************************************");
    log.info("DivideBy2Command init()");
  ...
  public Result execute(ChainContext chainContext) {
    ...
    log.info("DivideBy2Command execute()...");
    ...
    answer = new Integer(answer.intValue() / 2);
    chainContext.setAttribute("answer", answer);
    log.info("a, b, answer == " + chainContext.getAttribute("a") + ", " +
      chainContext.getAttribute("b") + ", " +
      chainContext.getAttribute("answer"));
    log.info("DivideBy2Command execute() done");
    return new Result(Result.SUCCESS);
  ...
  public Result cleanup(ChainContext chainContext) {
    ...
    log.info("DivideBy2Command cleanup()");
    log.info("**END****************************************\n\n");
    return new Result(Result.SUCCESS);
