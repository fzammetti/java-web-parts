...
/**
 * This class holds configuration information on each group of elements (usually
 * a group corresponds to a page, but it doesn't have to) that contains
...
public class AjaxGroup {
   ...
   * Log instance.
   */
  private static Log log = LogFactory.getLog(AjaxGroup.class);
  ...
  /**
   * Flag to determine if child element requests be asynchronous.  Note: must be
   * a string, rather than the more logical boolean, otherwise overriding
   * wouldn't work properly.  Defaults to "true".
   */
   ...
   *
   * @return The collection of error handlers for this group.
   */
  ...
  private boolean validate() {
    ...
    boolean valid = true;
    if (ajaxRef == null) {
      log.error("ajaxRef attribute of <group> element is required");
      valid = false;
    }
    ...
    if (!method.equalsIgnoreCase("head") &&
      ...
      log.error("method attribute of <group> element must be one of the " +
        "values: head, get, post, put, delete, trace, options, " +
